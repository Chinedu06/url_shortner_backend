...
class Url(db.Model):
    ...
    long_url = db.Column( db.String(60) , nullable=False  )
    short_url =  db.Column( db.String(60) , nullable=False , unique=True )
    url_code = db.Column(db.String(64) , nullable=False  )
    qr_code = db.Column(db.String(64) , nullable=True )
    created_at = db.Column(db.DateTime() , nullable=False , default=datetime.utcnow)
    user_id = db.Column(db.Integer() , db.ForeignKey('user.id') , nullable=True)
    ...
    @classmethod
    def get_this_month_user_link(cls, user_id):
        import datetime
        today = datetime.date.today()
        beginning_of_month = today.replace(day=1)
        count = cls.query.filter(cls.user_id == user_id, cls.created_at >= beginning_of_month).count()
        return count
